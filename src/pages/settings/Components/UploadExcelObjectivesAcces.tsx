...
const UploadExcelObjectivesAcces: FunctionComponent<{ data: Function }> = ({
  data,
}) => {
  ...
  const upload = async () => {
    ...
    setLoading(true);
    //upload Excel
    const responseUpload = await CRMRepository.uploadExcelObjectivesAccesories({
      file: fileExcel[0],
    ...
    const respObjectives: ObjectivesAccessories[] | null =
      await objectivesRepository.getObjectivesAccessories();
    if (respObjectives) {
      data(respObjectives);
    }
  ...
  return (
    <div style={{ position: 'relative' }}>
      <Link to="/excel/Template_accesorios_excel.xlsx" target="_blank">
...
export default UploadExcelObjectivesAcces;
